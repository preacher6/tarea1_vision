# -*- coding: utf-8 -*-
"""
Created on Mon Sep 17 10:31:43 2018
...
def interpolacion_bilineal(ruta_imagen, homografia):
    ...
    nueva_imagen = np.zeros([nuevas_filas, nuevas_columas, 3])
    ...
    for m in range(nuevas_filas):
        for n in range(nuevas_columas):
            ...
            - evaluar(q2) * (q4['x'] - p['x']) * (q1['y'] - p['y']) \
            - evaluar(q3) * (q1['x'] - p['x']) * (q4['y'] - p['y']) \
            + evaluar(q4) * (q1['x'] - p['x']) * (q1['y'] - p['y'])
            verificar = lambda q1, q2: (q1['x'] >= 0) and (q2['x'] <= (columnas - 1)) and (q1['y'] >= 0) and (
                        q2['y'] <= (filas - 1))
            punto['x'] = coordenada[0]+9
            punto['y'] = coordenada[1]+9
            ...
            if verificar(vertices[0], vertices[3]):
                nueva_imagen[m, n, :] = [int(elemento) for elemento in interpolar(punto, vertices[0], vertices[1], vertices[2], vertices[3])]
                #print(interpolar(punto, vertices[0], vertices[1], vertices[2], vertices[3]))

    #print(vertices)"""
    return nueva_imagen

imagen = "capilla60.jpg"
homografia = aplicar_homografia(puntos_p)
nueva = interpolacion_bilineal(imagen, homografia)
plt.imshow(nueva)
